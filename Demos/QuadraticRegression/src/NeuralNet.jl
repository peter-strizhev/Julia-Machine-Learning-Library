...
loaded_model = NNLib.LoadModel.load_model("trained_model.jld2")

dataPoints = 100

# Test the model on new data
X_test = rand(dataPoints, 1) * 10  # Generate new x values for the test set
X_test = (X_test .- mean(X_test)) ./ std(X_test)  # Scale them similarly to the training data

# Calculate true y values for test data
# sign = rand([-1, 1])
sign = 1
y_test = (sign .* X_test.^2) .+ 0.1 * randn(dataPoints, 1)
